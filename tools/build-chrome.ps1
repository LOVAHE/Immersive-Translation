...
if ([string]$SourceManifest.name -ne "Adaptive Translation") {
  throw "Unexpected Chrome manifest name: $($SourceManifest.name)"
}
if ([string]::IsNullOrWhiteSpace($Version)) {
  $Version = $ManifestVersion
} elseif ($Version -ne $ManifestVersion) {
  throw "Requested Chrome version $Version does not match manifest version $ManifestVersion."
}
$Stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$DistDir = Join-Path $Root "dist"
$StageDir = Join-Path $DistDir "chrome-$Version-$Stamp"
$ZipPath = Join-Path $DistDir "adaptive-translation-chrome-$Version-$Stamp.zip"
...
if (Test-Path -LiteralPath $ZipPath) {
  throw "Build zip already exists: $ZipPath"
}
...
Copy-Item -LiteralPath (Join-Path $Root "manifest.chrome.json") -Destination (Join-Path $StageDir "manifest.json")
...
if ($manifest.manifest_version -ne 3) {
  throw "Chrome package manifest must be Manifest V3."
}

$manifestText = Get-Content -Raw (Join-Path $StageDir "manifest.json")
foreach ($blocked in @("browser_specific_settings", '"scripts"', "preferred_environment")) {
  if ($manifestText.Contains($blocked)) {
    throw "Chrome package manifest contains blocked or store-risk field: $blocked"
  }
}
