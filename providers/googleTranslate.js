...
export class GoogleTranslate extends BaseTranslator {
  ...
  async translate({ text, sourceLang='auto', targetLang }) {
    ...
    const res = await withTimeout(fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }));
    L.info('status', res.status);
    if (!res.ok) { const t = await res.text().catch(()=> ''); L.error('error body', t.slice(0,500)); throw new Error(`Google API ${res.status}`); }
